...
def send_to_flows_controller(flow_id: str, account: str, run_id: str, task_id: str, status: str, message: str = None) -> str:
    ...
    try:
        ...
        response_data = {
            ...
            "task_id": task_id,
            "status": status,
            "step": "ms-extractor-sharepoint365", 
            "result": {
                "status": status,
                "message": message
            }
        ...
        if status == "failed" and message:
            response_data["message"] = [
                "MSExtractorSharepoint365 execution failed",
                message,
                f"Task: {task_id}",
                f"Account: {account}"
            ]
        ...
        logger.info(f"[MSExtractorSharepoint365] Respuesta enviada a flows-controller: {message_id}")
        logger.info(f"[MSExtractorSharepoint365] Task {task_id} status {status} reported to ms-flows-controller")
        
        return message_id
        
    except Exception as e:
        logger.error(f"[MSExtractorSharepoint365] Error enviando respuesta a ms-flows-controller: {e}")
        raise
