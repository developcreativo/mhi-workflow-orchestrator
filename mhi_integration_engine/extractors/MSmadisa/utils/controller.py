import json, os, time, io, logging,sys
...
class Controller:
    ...
    def __init__(self, account, extractor_name):
        ...
        try:
            ...
            self.project_id = self.__GCP_PROJECT_ID 
            self.__STORAGE_INTEGRATION_BUCKET = str(os.environ.get('_M_BUCKET_DATA'))
            #logger.info(f"_M_BUCKET_DATA: {self.__STORAGE_INTEGRATION_BUCKET}")
            self.__STORAGE_LASTEST_INCREMENTAL_BUCKET = str(os.environ.get('_M_BUCKET_LASTEST_EXECUTION'))
            #logger.info(f"_M_BUCKET_LASTEST_EXECUTION: {self.__STORAGE_LASTEST_INCREMENTAL_BUCKET}")
            ...
            msg=f"El proceso de instanciación ha fallado: {str(e)}"
            logger.error(msg)
            raise(msg)
    ...
    def saveStorage(self, dataset_name: str, 
        ...
        try:
            ...
            logger.info(f"Fichero correctamente almacenado en {gcs_path} con {len(data_frame)} filas")
            return gcs_path
    ...
    def getLatestSuccessExecution(self, dataset_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Warning: Could not get latest execution data: {str(e)}")
            return None
    ...
    def postLatestSuccessExecution(self, dataset_name: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Upload to GCS
            blob = bucket.blob(gcs_path)
            blob.upload_from_string(json_buffer.read(), content_type='application/json')
    ...
    def putNotification(
        ...
        #logger.info("Inicio del metodo de notificaciones")
        future = ''
    ...
    def _forceDataTypes(self, df: pd.DataFrame, dataset_name: str):
        ...
        try:
            ...
            if dataset_name in mapping_file:
                ...
                logger.info(f"No hay mapeo de tipos de datos para el dataset {dataset_name}")
                return df
                
        except Exception as e:
            logger.error(f"Error al buscar los mapeos: {str(e)}")
            return df  
        ...
        logger.info("Iniciamos el proceso de conversión de tipos")
        for col, dtype in conversions.items():
            #logger.info(f"Procesando campo {col} a tipo: {dtype}")
            try:
                ...
                logger.error(f"Error al convertir columna {col} a {dtype}: {str(e)}")
        logger.info("Finalizamos el proceso de conversión de tipos")
        return df
