    ...
    try:
        ...
        if status == "failed" and message:
            response_data["message"] = [
                "MSExtractorMadisa execution failed",
                message,
                f"Task: {task_id}",
                f"Account: {account}"
            ]
        ...
        logger.info(f"[MSExtractorMadisa] Respuesta enviada a flows-controller: {message_id}")
        logger.info(f"[MSExtractorMadisa] Task {task_id} status {status} reported to ms-flows-controller")
        
        return message_id
        
    except Exception as e:
        logger.error(f"[MSExtractorMadisa] Error enviando respuesta a ms-flows-controller: {e}")
        raise
