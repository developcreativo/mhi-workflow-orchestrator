    ...
    try:
        ...
        if status == "failed" and message:
            response_data["message"] = [
                "MSExtractorHolded execution failed",
                message,
                f"Task: {task_id}",
                f"Account: {account}"
            ]
        ...
        logger.info(f"[MSExtractorHolded] Respuesta enviada a flows-controller: {message_id}")
        logger.info(f"[MSExtractorHolded] Task {task_id} status {status} reported to ms-flows-controller")
        
        return message_id
        
    except Exception as e:
        logger.error(f"[MSExtractorHolded] Error enviando respuesta a ms-flows-controller: {e}")
        raise
