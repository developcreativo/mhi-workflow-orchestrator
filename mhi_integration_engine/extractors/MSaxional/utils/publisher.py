    ...
    try:
        ...
        if status == "failed" and message:
            response_data["message"] = [
                "MSExtractorAxional execution failed",
                message,
                f"Task: {task_id}",
                f"Account: {account}"
            ]
        ...
        logger.info(f"[MSExtractorAxional] Respuesta enviada a flows-controller: {message_id}")
        logger.info(f"[MSExtractorAxional] Task {task_id} status {status} reported to ms-flows-controller")
        
        return message_id
        
    except Exception as e:
        logger.error(f"[MSExtractorAxional] Error enviando respuesta a ms-flows-controller: {e}")
        raise
