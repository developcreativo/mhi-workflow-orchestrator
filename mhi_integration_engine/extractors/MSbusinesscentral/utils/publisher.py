...
def send_to_flows_controller(flow_id: str, account: str, run_id: str, task_id: str, status: str, message: str = None) -> str:
    ...
    try:
        ...
        response_data = {
            ...
            "task_id": task_id,
            "status": status,
            "step": "ms-extractor-businesscentral", 
            "result": {
                "status": status,
                "message": message
            }
        ...
        if status == "failed" and message:
            response_data["message"] = [
                "MSExtractorBusinessCentral execution failed",
                message,
                f"Task: {task_id}",
                f"Account: {account}"
            ]
        ...
        logger.info(f"[MSExtractorBusinessCentral] Respuesta enviada a flows-controller: {message_id}")
        logger.info(f"[MSExtractorBusinessCentral] Task {task_id} status {status} reported to ms-flows-controller")
        
        return message_id
        
    except Exception as e:
        logger.error(f"[MSExtractorBusinessCentral] Error enviando respuesta a ms-flows-controller: {e}")
        raise
