...
def send_to_flows_controller(flow_id: str, account: str, run_id: str, task_id: str, status: str, message: str = None) -> str:
    ...
    """
    print(f"[DEBUG] send_to_flows_controller llamado con: flow_id={flow_id}, account={account}, run_id={run_id}, task_id={task_id}, status={status}")
    try:
        ...
        if status == "failed" and message:
            response_data["message"] = [
                "MS UlysesCloud execution failed",
                message,
                f"Task: {task_id}",
                f"Account: {account}"
            ]
        ...
        logger.info(f"[MSExtractorUlysesCloud] Respuesta enviada a flows-controller: {message_id}")
        logger.info(f"[MSExtractorUlysesCloud] Task {task_id} status {status} reported to ms-flows-controller")
        
        return message_id
        
    except Exception as e:
        logger.error(f"[MSExtractorUlysesCloud] Error enviando respuesta a ms-flows-controller: {e}")
        raise
