...
class Controller:
    ...
    def saveStorage(self, dataset_name: str, 
                     file_name: str, data_frame: pd.DataFrame, escenario: str) -> str:
        """
        ...
        try:
            ...
            gcs_path = f'gs://{self.__STORAGE_INTEGRATION_BUCKET}/{data_source}/{dataset_name}/{escenario.lower()}/{file_name}'
    ...
    def getLatestSuccessExecution(self, dataset_name: str, escenario: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            # Define the GCS path
            gcs_path = f"{self._extractor_name}/{dataset_name}/{self._m_account}_{escenario.lower()}.json"
    ...
    def postLatestSuccessExecution(self, dataset_name: str, data: Dict[str, Any], escenario: str) -> None:
        """
    ...
    def _forceDataTypes(self, df: pd.DataFrame, dataset_name: str):
        ...
        try:
            ...
            if dataset_name in mapping_file:
                ...
                logger.warning(f"No hay mapeo de tipos de datos para el dataset {dataset_name}")
                return df
        ...
        for col, dtype in conversions.items():
            #logger.info(f"Procesando columana {col} a tipo: {dtype}")
            try:
