...
def send_to_flows_controller(flow_id: str, account: str, run_id: str, task_id: str, status: str, message: str = None) -> str:
    ...
    try:
        ...
        logger.info(f"[KPI-ENGINE] Respuesta enviada a flows-controller: {message_id}")
        logger.info(f"[KPI-ENGINE] Task {task_id} status {status} reported to ms-flows-controller")
        
        return message_id
        
    except Exception as e:
        logger.error(f"[KPI-ENGINE] Error enviando respuesta a ms-flows-controller: {e}")
        raise
